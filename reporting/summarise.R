...
columns <- c("workflow", "run_id", "task", "shards", "attempts",
             "job_id", "status", "exit_code", "submit_ts", "start_ts",
             "end_ts", "wall_time", "cpu_time")
...
summarise(
  ...
  complete_pc = 100 * complete / coalesce(first(expected), n()),
...
if (isatty(stdout()) & isatty(stderr())) {
  ...
  output_headers <- c("Workflow", "Run ID", "Task", "Complete", "CPU Time",
                      "Pending", "Running", "Failed", "Other")
